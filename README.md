...
まず **[リリースページ](../../releases/latest)** から `SetupScripts.zip` をダウンロードし、**[Mod 適用手順書](manual/mod-setup.md)** の手順でセットアップを行ってください。

セットアップ完了後、以下の機能を利用できます。

| 機能 | 概要 | 手順書 |
|---|---|---|
| お題一覧表示 | 試合中に選択されたジャンル・お題の一覧を確認する | [説明書](manual/mod-show-theme-list.md) |
| カスタムお題 CSV 入力 | CSV ファイルでカスタムお題を入力する（最大100文字） | [説明書](manual/mod-custom-theme.md) |
| 配信者モード | 役割を画面から秘匿し、配信中に安全にプレイする | [説明書](manual/mod-conceal-role.md) |
| WordWolf モード | インポスターに異なるお題を配布してゲームを楽しむ | [説明書](manual/mod-conceal-role.md) |
...
root
  ...
  ├── Manual_JP                       <--- Modの適用手順書や各Modの説明書（PDF版）がある
  ├── manual                          <--- Modの適用手順書や各Modの説明書（Markdown版）がある
  ├── plugins                         
